            ...
            for target_slug_dir in Path(backup_base_path).iterdir():
                ...
                for date_dir in target_slug_dir.iterdir():
                    ...
                    for file_path in date_dir.iterdir():
                        ...
                        # Only finalized artifacts with valid recovery metadata
                        # are exposed to destructive restore workflows.
                        sidecar_data = read_backup_sidecar(artifact_path)
                        sidecar_artifact = (
                            sidecar_data.get("artifact_path") if sidecar_data is not None else None
                        )
                        if (
                            sidecar_data
                            and isinstance(sidecar_artifact, str)
                            and Path(sidecar_artifact).resolve() == file_path.resolve()
                        ):
                            ...
                            artifacts.append(
                                BackupFromDisk(
                                    ...
                                    date=(
                                        sidecar_data.get("created_at", "").split("T")[0]
                                        if sidecar_data.get("created_at")
                                        else date_str
                                    ),
