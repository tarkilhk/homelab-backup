...
class RadarrPlugin(BackupPlugin):
    """Radarr backup plugin using built-in backup API.

    Research notes: Radarr's UI exposes backups via System → Backup which
    generates a zip archive for download. This plugin automates that flow by
    calling the backup endpoints with an API key for authentication.

    Correct API flow (Servarr family, including Radarr v3):
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if resp.status_code != 200:
                    raise RuntimeError(f"Radarr API returned status {resp.status_code}")
                return True
        except RuntimeError:
            raise
        except httpx.HTTPError as exc:  # pragma: no cover - network errors
            self._logger.warning("radarr_test_error | url=%s error=%s", status_url, exc)
            raise ConnectionError(f"Failed to connect to Radarr server: {exc}") from exc
    ...
    async def backup(self, context: BackupContext) -> Dict[str, Any]:
        ...
        self._logger.info(
            "radarr_backup_trigger | job_id=%s target_id=%s url=%s artifact=%s",
            context.job_id,
            context.target_id,
            command_url,
            "<pending>",
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
            ...
            try:
                ...
                self._logger.error(
                    "radarr_backup_trigger_error | job_id=%s target_id=%s error=%s",
                    context.job_id,
                    context.target_id,
                    str(exc),
                )
            ...
            if backup_id is None:
                msg = "Unable to locate newly created Radarr backup entry after trigger" + (
                    f" | last_error={last_list_error}" if last_list_error else ""
                )
                self._logger.error(
                    "radarr_backup_list_timeout | job_id=%s target_id=%s msg=%s",
                    context.job_id,
                    context.target_id,
                    msg,
                )
            ...
            if backup_path:
                if backup_path.startswith("/"):
                    ...
                    direct_url = f"{base_url}/{backup_path}"
                self._logger.info(
                    "radarr_backup_download_path | job_id=%s target_id=%s url=%s",
                    context.job_id,
                    context.target_id,
                    direct_url,
                )
                try:
                    ...
                    self._logger.warning(
                        "radarr_backup_download_path_error | job_id=%s target_id=%s error=%s",
                        context.job_id,
                        context.target_id,
                        str(exc),
                    )
            ...
            self._logger.info(
                "radarr_backup_download | job_id=%s target_id=%s url=%s backup_id=%s",
                context.job_id,
                context.target_id,
                download_url,
                backup_id,
            ...
            try:
                ...
                self._logger.error(
                    "radarr_backup_download_error | job_id=%s target_id=%s error=%s",
                    context.job_id,
                    context.target_id,
                    str(exc),
                )
            ...
            if not fallback_url:
                raise RuntimeError(
                    f"Radarr backup download failed for id={backup_id} and no fallback path available"
                )

            self._logger.info(
                "radarr_backup_download_fallback | job_id=%s target_id=%s url=%s",
                context.job_id,
                context.target_id,
                fallback_url,
            )
            try:
                ...
                if not content2:
                    raise RuntimeError("Radarr backup fallback download returned no content")
                return publish(content2)
            except httpx.HTTPError as exc:
                self._logger.error(
                    "radarr_backup_download_fallback_error | job_id=%s target_id=%s error=%s",
                    context.job_id,
                    context.target_id,
                    str(exc),
                )
        ...
        raise RuntimeError("Radarr backup completed without an artifact")

    async def restore(self, context: RestoreContext) -> Dict[str, Any]:
        """Restore a Radarr backup.

        Note: Radarr manages its own backup restoration. This function copies the
        backup file to a restore directory. To complete the restore:
        1. Access Radarr UI → System → Backup
        2. Upload the backup ZIP file from the restore location
        3. Radarr will handle the restoration process

        Alternatively, you can copy the backup to Radarr's backup directory
