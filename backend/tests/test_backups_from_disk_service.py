...
from app.services.backups_from_disk import BackupFromDisk, BackupsFromDiskService
...
def test_scan_backups_basic(backup_dir, db_session: Session):
    ...
    # Artifact1 has no sidecar and is not exposed as recoverable.
...
def test_scan_backups_includes_tracked(backup_dir, db_session: Session):
    ...
    artifact1_path = str(
        backup_dir / "target1" / "2025-01-15" / "pihole-backup-20250115T120000.zip"
    )
    ...
    # A database row does not replace required recovery metadata.
    assert len(backups) == 1
    backup_paths = [b.artifact_path for b in backups]
    assert not any("pihole-backup" in p for p in backup_paths)
