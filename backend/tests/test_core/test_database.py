...
def test_get_session_initializes_engine_if_needed(mock_db_dir):
    ...
    from app.core.db import SessionLocal, _engine
...
def test_bootstrap_db_handles_existing_data(mock_db_dir):
    ...
    target = Target(
        name="Test Target", slug="test-target", plugin_name="dummy", plugin_config_json="{}"
    )
...
def test_database_session_isolation(mock_db_dir):
    ...
    try:
        ...
        result = session2.execute(
            text("SELECT name FROM sqlite_master WHERE type='table' AND name='test_isolation'")
        )
...
def test_database_concurrent_access(mock_db_dir):
    ...
    try:
        ...
        for i, (session_gen, session) in enumerate(sessions):
            ...
            target = Target(
                name=f"Target {i}", slug=f"target-{i}", plugin_name="dummy", plugin_config_json="{}"
            )
            session.add(target)
            session.commit()
...
def test_historical_runs_migration_makes_job_nullable_without_data_loss():
    """The documented restore migration must transform an existing SQLite DB."""
    engine = create_engine("sqlite://", poolclass=StaticPool)
    with engine.begin() as conn:
        conn.exec_driver_sql("PRAGMA foreign_keys=ON")
        conn.exec_driver_sql("CREATE TABLE jobs (id INTEGER PRIMARY KEY)")
        conn.exec_driver_sql("CREATE TABLE targets (id INTEGER PRIMARY KEY)")
        ...
        conn.exec_driver_sql("INSERT INTO jobs (id) VALUES (1)")
        conn.exec_driver_sql("INSERT INTO targets (id) VALUES (1)")
        conn.exec_driver_sql(
            "INSERT INTO runs (id, job_id, started_at, status, operation) "
            "VALUES (1, 1, CURRENT_TIMESTAMP, 'success', 'backup')"
        )
        conn.exec_driver_sql("INSERT INTO target_runs (id, run_id, target_id) VALUES (1, 1, 1)")

    _migrate_runs_job_id_nullable(engine)

    with engine.begin() as conn:
        columns = {row[1]: row for row in conn.exec_driver_sql("PRAGMA table_info(runs)")}
        assert columns["job_id"][3] == 0
        assert conn.exec_driver_sql("SELECT COUNT(*) FROM runs").scalar_one() == 1
        assert conn.exec_driver_sql("SELECT COUNT(*) FROM target_runs").scalar_one() == 1
        conn.exec_driver_sql(
            "INSERT INTO runs (id, job_id, started_at, status, operation) "
            "VALUES (2, NULL, CURRENT_TIMESTAMP, 'success', 'restore')"
        )
