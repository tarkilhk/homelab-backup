...
def test_runs_filters_status_date_target(
    client: TestClient, db_session_override: Session
) -> None:
    """Verify runs list endpoint supports status, date range, and target filters."""
    # Create two targets
    r = client.post(
        "/api/v1/targets/",
        json={"name": "T1", "slug": "t1", "plugin_name": "dummy", "plugin_config_json": "{}"},
    )
    assert r.status_code == 201
    t1_id = r.json()["id"]
    ...
    r = client.post(
        "/api/v1/targets/",
        json={"name": "T2", "slug": "t2", "plugin_name": "dummy", "plugin_config_json": "{}"},
    )
    assert r.status_code == 201
    t2_id = r.json()["id"]
    ...
    r = client.post(
        "/api/v1/jobs/",
        json={"target_id": t1_id, "name": "Job1", "schedule_cron": "0 1 * * *", "enabled": "true"},
    )
    assert r.status_code == 201
    j1_id = r.json()["id"]
    ...
    r = client.post(
        "/api/v1/jobs/",
        json={"target_id": t2_id, "name": "Job2", "schedule_cron": "0 2 * * *", "enabled": "true"},
    )
    assert r.status_code == 201
    j2_id = r.json()["id"]
    ...
    assert r.status_code == 200
    items = r.json()
    # All items should belong to job j1 (target t1)
    assert all(it["job"]["target_id"] == t1_id for it in items)
