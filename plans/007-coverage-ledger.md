...
| Shared PostgreSQL databases | `planned-plugin` | Plan 017 is revalidating the generic per-database PostgreSQL 16 foundation with private authentication, exact archive provenance, and two fresh transactional restore rounds. All authoritative production DB targets/jobs still need final enumeration. |
