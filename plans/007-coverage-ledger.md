...
| Vaultwarden | `verified-plugin` | Exact 1.37.1 stop-based A/B backups and four independent fresh restores passed across two clean rounds, including Web Vault secure-note and attachment recovery before and after restart. Production `v0.4.0` is deployed with the immutable image manifest and approved brief stop/start; target/job `1` passed preflight and backup Run/TargetRun `264`/`263` published a sidecar-discovered 601,253-byte artifact with recorded SHA-256. File Sends are unused, and production restore remains forbidden. |
