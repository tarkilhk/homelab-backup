...
| Jellyfin (legacy; Plan 021 blocked) | `jellyfin/jellyfin:10.11.11` (research resolved index `sha256:aefb67e6a7ff1debdd154a78a7bbb780fd0c873d8639210a7f6a2016ad2b35db`, linux/amd64 manifest `sha256:0b901391a662862eddb5dc55d244d7883cbb6236ef5b9a6ea82abc78a89819f0`) | The v0.2.1 baseline recorded connectivity and two native archives, but not a coherent complete `/config` boundary, strict validation, or least privilege. Exact source proves all API keys are unrestricted administrators and native database/file capture has no shared transaction | The baseline recorded one official restore plus restart/readiness, not restored semantic state or two fresh rounds. Native restore has no terminal result/rollback and the archive omits plugins, plugin configuration, device identity, and other state; revalidation awaits explicit boundary, downtime, inventory, authority, and restore-claim decisions |
| Lidarr | `ghcr.io/linuxserver/lidarr:3.1.0.4875-ls38` (drill pinned linux/amd64 manifest `sha256:0199ff56d973da7b66158ba8823cf3eac905d47b6ab7524d213931debfa75225`) | Four exact native SQLite backups across two clean rounds were uniquely attributed, copied by an unprivileged runner from the fixed read-only mount, strictly validated, privately published with structural sidecars, and removed from the native source only after durable publication | Four fresh RestoreService destinations proved phase-specific tags and control-plane structure through two restart/content cycles each; capability is `automatic` for the Lidarr control plane, while music and download data remain external |
| MySQL (legacy; Plan 019 blocked) | `mysql:8.4.0` and `mysql:8.4.0-oraclelinux8` | The v0.2.1 baseline recorded connectivity and two logical dumps, but not current-contract artifact validation. Exact pinned MySQL Shell 8.4.0 revalidation wrote bytes while warning that consistency could not be guaranteed without broader role/system/global authority | The baseline recorded one empty isolated import plus `mysqlcheck`; no current-contract two-round restore proof exists. Generic online revalidation is blocked pending an explicit broader-privilege or quiescence decision; Invoice Ninja retains its separate native boundary and Standard Notes its quiesced composite gate |
| Pi-hole (legacy; Plan 020 blocked) | `pihole/pihole:2026.07.2` (research resolved index `sha256:f7d1be836e3bc608b56d82fc9904f5a831cdfbc0dc9c6d58f94e4c985c70038b`, linux/amd64 manifest `sha256:7c96327ecfb96dbc74b0a47d073dbef7d60526e0aa87519b2a2f7a0007cb5c88`) | The v0.2.1 baseline recorded v6 SID authentication and two Teleporter exports, but not current-contract atomicity or strict artifact validation. Exact source proves export is sequential across files/tables and the narrowest application password retains unrelated write authority | The baseline recorded one isolated import plus a post-import export, not two complete recovery rounds or semantic/DNS proof. Revalidation is blocked pending explicit consistency and source-authority decisions |
...
| Radarr | `ghcr.io/linuxserver/radarr:6.3.0.10514-ls313` (drill pinned linux/amd64 manifest `sha256:263be1036419fcb38fc1cf76be90db8db4b0dc49fd492617b17cc58e9e0bf1b5`) | Four exact native SQLite backups across two clean rounds were uniquely attributed, copied by an unprivileged runner from the fixed read-only mount, strictly validated, privately published with structural sidecars, and removed from the native source only after durable publication | Four fresh RestoreService destinations proved phase-specific tags and control-plane structure through two restart/content cycles each; capability is `automatic` for the Radarr control plane, while movies and download data remain external |
...
inventory proved both lazily created children were legitimately absent. The
plugin now represents that shape as explicit empty archive roots only while the
metadata mount itself is present and safe; focused tests and two expanded clean
exact-image drills pass. It still does not need
